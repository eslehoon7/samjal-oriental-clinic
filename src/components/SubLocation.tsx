...
export default function SubLocation() {
  ...
  const [galleryItems, setGalleryItems] = useState(() => {
    const defaultItems = [
      ...
      },
      {
        ...
        desc: "오직 환자 한 분만을 위한 아늑하고 멸균 처리된 치료 환경입니다. 대관절 동기침법 및 심부 온열 요법에 한결 깊이 스며들 수 있도록 정밀하게 원내 고유 설계했습니다.",
  ...
  useEffect(() => {
    localStorage.setItem("samjal_gallery_items", JSON.stringify(galleryItems));
  }, [galleryItems]);
      ...
      <div className="relative w-full h-[380px] sm:h-[480px] bg-[#0F172A] overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-20 text-center space-y-3 px-4 animate-fadeIn pt-16">
          ...
          <p className="text-slate-300 font-sans text-sm sm:text-base max-w-lg mx-auto tracking-wide leading-relaxed font-light">
            가장 가까운 곳에서 전하는 국가대표급 전임 원장단<br /> 정성의 수준높은 메디컬 서비스를 약속드립니다.
          </p>
        </div>
      </div>
        ...
        <div id="samjal-interior-tour" className="mb-16 pt-10 pb-12 border-t border-b border-slate-200/80 animate-fadeIn text-left relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div className="space-y-2 text-center sm:text-left">
                ...
                <p className="text-xs sm:text-sm font-sans text-slate-500 leading-relaxed">
                  마우스를 올리시면 가동 중인 롤링이 멈추며 한층 선명하고 크게 보실 수 있습니다.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="relative w-full overflow-hidden py-6">
            ...
            <div className="interior-marquee-track gap-6">
              {/* Duplicate array mapping to handle infinite continuous horizontal flow seamlessly */}
